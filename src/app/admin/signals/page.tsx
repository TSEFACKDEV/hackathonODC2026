...
import { MdCheckCircle, MdCancel, MdFilterList, MdLocationOn, MdPerson, MdLocalShipping, MdAccessTime } from "react-icons/md";
import api from "@/utils/api";
import EmptyState from "@/components/ui/EmptyState";

const STATUS_OPTS = ["ALL","PENDING","IN_PROGRESS","COLLECTED","REJECTED"];
const sConfig: Record<string,{label:React.ReactNode;cls:string}> = {
  PENDING:{label:<><MdAccessTime className="inline mr-1" size={14}/>En attente</>,cls:"badge-yellow"},
  IN_PROGRESS:{label:<><MdLocalShipping className="inline mr-1" size={14}/>En cours</>,cls:"badge-blue"},
  COLLECTED:{label:<><MdCheckCircle className="inline mr-1" size={14}/>Collecté</>,cls:"badge-green"},
  REJECTED:{label:<><MdCancel className="inline mr-1" size={14}/>Rejeté</>,cls:"badge-red"},
};
          ...
          {signals.map((sig, i) => (
            <motion.div key={sig.id} initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ delay:i*0.04 }}
              ...
              <div className="flex-1 min-w-0">
                ...
                {sig.user && <p className="text-xs text-gray-400 mt-1"><MdPerson className="inline mr-1" size={12}/> {sig.user.name}</p>}
                <div className="flex gap-1.5 mt-2 flex-wrap">
                  {sig.status === "PENDING" && <>
                    <button onClick={() => updateStatus(sig.id,"IN_PROGRESS")}
                      ...
                      <MdLocalShipping size={14}/> En cours
                    </button>
